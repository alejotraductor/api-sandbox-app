/* tslint:disable:all */
/**
 * ITransferService file.
 */
import { Observable } from "rxjs";
import { BankListResponse } from "./../../lib/types/bank_list_response";
import { TokenResponse } from "./../../lib/types/remote/token_response";
import { TransferTokenRequest } from "./../../lib/types/remote/transfer_token_request";
export interface ITransferService {
    /**
     *  Transfer Token
     * @param tokenRequest - transfer tokens request body
     * @param mid - public merchant id
     * @param isTest - is test environment
     * @param regional - Define if endpoint used regional URL
     */
    requestTransferToken(tokenRequest: TransferTokenRequest, mid: string, isTest: boolean, regional: boolean): Observable<TokenResponse>;
    /**
     *  Bank List
     * @param mid - public merchant id
     * @param isTest - is test environment
     * @param regional - Define if endpoint used regional URL
     */
    requestPseBankList(mid: string, isTest: boolean, regional: boolean): Observable<BankListResponse>;
}
